...
本项目采用 AI 辅助编程模式，通过 skills 机制实现高效、规范的代码开发流程：
...
```
需求分析 → TodoWrite 规划任务 → 执行开发 → 测试验证 → 代码审查 → 部署上线
     │                              │
     └───────── 循环迭代 ───────────┘
```
...
1. **规范约束**：遵循项目现有代码风格和命名规范
2. **架构一致性**：使用统一的技术栈和设计模式
3. **依赖检查**：优先使用项目已有的库和工具
4. **安全检查**：敏感信息通过环境变量配置，不硬编码
5. **测试验证**：代码修改后执行单元测试和集成测试

### 🚫 防代码跑偏策略

- **代码审查**：重大修改前进行 Code Review
- **测试覆盖**：核心功能必须有单元测试覆盖
- **配置隔离**：敏感配置与代码分离，使用 `.example` 文件
- **分支管理**：使用 feature 分支开发，定期合并到主分支
